...
AllPathwaysKEGGPS <- read.delim("C:/Users/bruep/OneDrive/Área de Trabalho/Kleiton/Pathways/Artigo_suplementares/Supplementary_Dataset_2_txt.txt", header=T)
...
ABSPathwayScore2 <- as.data.frame(LUSCABSScoreSum(LUSCTCGATvNDEG))
...
# Assuming ABSPathwayScore2 and Pvals are already loaded data.frames

# Transpose both data frames
ABSPathwayScore2_t <- t(ABSPathwayScore2)
Pvals_t <- t(Pvals)

# Convert the transposed matrices into data frames
ABSPathwayScore2_t <- as.data.frame(ABSPathwayScore2_t)
Pvals_t <- as.data.frame(Pvals_t)

# Replace dots with spaces in the row names (which will be the pathway names in the new data frame)
rownames(ABSPathwayScore2_t) <- gsub("\\.", " ", rownames(ABSPathwayScore2_t))

# Create a new data frame that combines the transposed ABSPathwayScore2 and Pvals data
df_combined <- data.frame(
  Pathway = rownames(ABSPathwayScore2_t),  # Pathway names from row names
  ABSScore = ABSPathwayScore2_t$V1,  # Column with ABS Pathway Scores
  Pvals = Pvals_t$V1  # Column with P-values
)

# Set column names of the new data frame to be from ABSPathwayScore2
colnames(df_combined) <- c("Pathway", "ABS Pathway Score", "P-values")

# Check the structure of the combined dataframe
head(df_combined)

# Exclude rows where P-values are equal to zero
df_combined_clean <- df_combined %>%
  filter(`P-values` != 0)

# Sort the data frame by ABS Pathway Score in descending order
df_combined_clean <- df_combined_clean %>%
  arrange(desc(`ABS Pathway Score`))

# Check the structure of the cleaned and sorted dataframe
head(df_combined_clean)

###### Plot Top 10 Metabolic Pathways
library(ggplot2)

# Select top 10 rows based on the Pathway Score
df_top10 <- head(df_combined_clean, 10)

# Select top 10 rows based on the Pathway Score
df_top10 <- head(df_combined_clean, 10)

# Set the file to save the plot, using a higher resolution and larger dimensions
png(filename = "Top10_LUSC_Metabolic_Pathways.png", width = 6000, height = 4000, res = 300)

# Create the plot for the Top 10 pathways
ggplot(df_top10, aes(x = reorder(Pathway, `ABS Pathway Score`), y = `ABS Pathway Score`)) +
  geom_bar(stat = "identity", fill = "red") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Top 10 LUSC Metabolic Pathways",
       x = "Metabolic Pathways",
       y = "Pathway Score") +
  theme(axis.text.y = element_text(size = 24),  # Increase y-axis label size
        ...
        plot.title = element_text(size = 34, hjust = 0.5), # Increase plot title size
        axis.line = element_line(size = 1),     # Increase thickness of axis lines
        axis.ticks = element_line(size = 1))    # Increase thickness of ticks on both axes

# Turn off the graphic device to save the file
dev.off()


# Assuming df_top10 is already created and has a column of P-values named 'Pvals_t.V1'

# Adding the new column 'log(pvalue)' as -log10 of the P-values
df_top10$`-log10(pvalue)` <- -log10(df_top10$`P-values`)

# Verify the new column in the dataframe
print(df_top10)

###### Plot P-values of Top 10 Metabolic Pathways
# Set the file to save the plot, using a higher resolution and larger dimensions
png(filename = "Top10_LUSC_Metabolic_Pathways_Pvalues.png", width = 6000, height = 4000, res = 300)

# Create the plot for the Top 10 pathways
ggplot(df_top10, aes(x = reorder(Pathway, `-log10(pvalue)`), y = `-log10(pvalue)`)) +
